...
describe("MockUsdPriceFeed", function () {
  ...
  it("returns zero round data before any update", async () => {
    ...
    expect(data.roundId).to.eq(0n);
    expect(data.answer).to.eq(0n);
    expect(data.startedAt).to.eq(0n);
    expect(data.updatedAt).to.eq(0n);
    expect(data.answeredInRound).to.eq(0n);
  });
  ...
  it("stores and overwrites full round data", async () => {
    ...
    expect(data.roundId).to.eq(1n);
    expect(data.answer).to.eq(ethers.parseUnits("600", 8));
    expect(data.startedAt).to.eq(10n);
    expect(data.updatedAt).to.eq(11n);
    expect(data.answeredInRound).to.eq(1n);

    await feed.setRoundData(2n, -1n, 20n, 21n, 1n);
    data = await feed.latestRoundData();
    expect(data.roundId).to.eq(2n);
    expect(data.answer).to.eq(-1n);
    expect(data.startedAt).to.eq(20n);
    expect(data.updatedAt).to.eq(21n);
    expect(data.answeredInRound).to.eq(1n);
  });
});
