...
-- Import this in Supabase before using the recruiter portal setSquadImage action.

alter table public.recruiters
add column if not exists squad_image_url text;

alter table public.recruiters
drop constraint if exists recruiters_squad_image_url_check;

alter table public.recruiters
add constraint recruiters_squad_image_url_check
check (
...
comment on column public.recruiters.squad_image_url is
