...
class Store {
  ...
  retryable(id) { const row = this.rows.get(id); row.state = 'CLAIMABLE'; this.release(id); }
}
...
async function claim({ store, chain, id, wallet, expectedChain, crashAfterPayment = false }) {
  ...
  if (crashAfterPayment) { store.retryable(id); return { ok: false, code: 'SIMULATED_CRASH' }; }
  store.claimed(id, tx);
  return { ok: true, tx };
}
...
async function crashRecovery() {
  const store = new Store(); const chain = new FakeChain();
  const id = 'solana:airdrop_trader:cert-001:WalletABC';
...
await crashRecovery();
await isolation();
console.log('PASS incentive-cert self-test: concurrency, crash recovery, chain/wallet isolation');
