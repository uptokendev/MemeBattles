...
const evidence = (root, files) => files.slice(0, 5).map((file) => path.relative(root, file)).join(', ');
...
export async function runSourceAudit({ root = process.cwd(), report }) {
  ...
  const idempotency = await find(files, /CLAIMING|claimed_at|idempot/i);
  report.add('Claim idempotency/recovery markers found', idempotency.length ? 'PASS' : 'WARN', { evidence: evidence(root, idempotency) }, 'shared');

  // Do not flag the EVM weekly-airdrop runner for rejecting Solana. That is intentional
  // because Solana uses a separate settlement rail. Only explicit generic-claim disable
  // markers should block the Solana claim certification here.
