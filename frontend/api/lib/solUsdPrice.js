/**
 * SOL/USD for league prize USD estimates.
 *
 * Priority:
 * 1) SOL_USD_PRICE or LEAGUE_SOL_USD_PRICE env
 * 2) Binance SOLUSDT spot, cached in-process
 *
 * Set SOL_USD_PRICE_FETCH=0 to disable network fetch.
 */

const CACHE_TTL_MS = Math.max(15_000, Number(process.env.SOL_USD_PRICE_CACHE_MS || 60_000) || 60_000);
const FETCH_ENABLED = !["0", "false", "no", "off"].includes(
  String(process.env.SOL_USD_PRICE_FETCH || "1").trim().toLowerCase(),
);

let cache = { price: 0, at: 0, source: "none" };

function envPrice() {
  const n = Number(process.env.SOL_USD_PRICE || process.env.LEAGUE_SOL_USD_PRICE || 0);
  return Number.isFinite(n) && n > 0 ? n : 0;
}

async function fetchSpotSolUsd() {
...
export async function resolveSolUsdPrice() {
  ...
  const spot = await fetchSpotSolUsd();
  if (spot > 0) {
...
export function readSolUsdPriceSync() {
  const fromEnv = envPrice();
