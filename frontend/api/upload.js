...
export default async function handler(req, res) {
  ...
  const rawAddress = String(q.address || "");
  // Only lowercase EVM-style addresses (0x...) for path consistency.
  // Solana base58 pubkeys must keep their original casing.
  ...
  try {
    ...
    if (!res.headersSent) {
      return bad(res, 400, e?.message || "Upload failed");
    }
  }
}
