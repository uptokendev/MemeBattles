...
const STATUSES = new Set(["draft", "promotion_published", "ready_to_launch", "deployed", "archived"]);
...
function normalizeTicker(value) {
  return String(value || "").trim().replace(/^\$+/, "").replace(/[^a-zA-Z0-9]/g, "").toUpperCase().slice(0, 12);
}
...
function slugify(value) {
  const base = String(value || "draft").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 48);
  return base || "draft";
}
...
function memoryStore() {
  if (!globalThis.__mwz_prepare_mode_store) {
    ...
    globalThis.__mwz_prepare_mode_store = {
      ...
      metrics: new Map([[demoDraft.id, { ...ZERO, views: 6294, follows: 412, comments: 127, reactions: 219, shares: 93, signedActions: 1843 }]]),
    };
  }
...
export async function drafts(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const store = memoryStore();
    const items = Array.from(store.drafts.values()).filter((draft) => owner ? draft.creatorWallet === owner : draft.visibility === "public" && draft.status !== "archived").sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)));
  ...
  if (pool) {
    ...
    const inserted = await pool.query(`insert into campaign_drafts
      ...
      chainId, creatorWallet, name, ticker, cleanText(body.description, 1200) || null, cleanText(body.category, 40) || "meme",
      cleanUrl(body.logoUrl) || null, cleanUrl(body.websiteUrl) || null, cleanUrl(body.xUrl) || null, cleanUrl(body.otherUrl) || null,
      makeSlug(name, ticker), visibility,
    ]);
  ...
  const draft = {
    id: randomUUID(), chainId, creatorWallet, name, ticker,
    ...
    slug: makeSlug(name, ticker), status: "draft", visibility,
    campaignAddress: null, tokenAddress: null, deployTxHash: null, archivedAt: null, deployedAt: null, createdAt: now, updatedAt: now,
  };
...
export async function draftPromotion(req, res) {
  ...
  const promotion = {
    ...
    telegramUrl: cleanUrl(body.telegramUrl), discordUrl: cleanUrl(body.discordUrl), xUrl: cleanUrl(body.xUrl), websiteUrl: cleanUrl(body.websiteUrl),
    docs: cleanStringArray(body.docs, 8, 500), creatorNote: cleanText(body.creatorNote, 3000), bannerUrl: cleanUrl(body.bannerUrl), shareMessage: cleanText(body.shareMessage, 500),
  };

  const pool = await getPool();
  if (pool) {
    const exists = await pool.query("select id from campaign_drafts where id::text = $1 limit 1", [id]);
    ...
    await pool.query(`insert into campaign_draft_promotion
      ...
      on conflict (draft_id) do update set
        mission_statement = excluded.mission_statement, roadmap = excluded.roadmap, launch_strategy = excluded.launch_strategy,
        telegram_url = excluded.telegram_url, discord_url = excluded.discord_url, x_url = excluded.x_url, website_url = excluded.website_url,
        docs = excluded.docs, creator_note = excluded.creator_note, banner_url = excluded.banner_url, share_message = excluded.share_message,
        published_at = coalesce(excluded.published_at, campaign_draft_promotion.published_at), updated_at = now()`, [
      id, promotion.missionStatement, JSON.stringify(promotion.roadmap), promotion.launchStrategy, promotion.telegramUrl, promotion.discordUrl,
      promotion.xUrl, promotion.websiteUrl, JSON.stringify(promotion.docs), promotion.creatorNote, promotion.bannerUrl, promotion.shareMessage, publish ? now : null,
    ]);
...
export async function draftFollow(req, res) {
  ...
  set.add(wallet); store.follows.set(id, set);
  const metrics = store.metrics.get(id) || { ...ZERO };
  metrics.follows = set.size; metrics.signedActions += 1; store.metrics.set(id, metrics);
...
export async function draftComments(req, res) {
  ...
  if (req.method === "GET") {
    const pool = await getPool();
    if (pool) {
      const result = await pool.query("select * from campaign_draft_comments where draft_id = $1 and moderation_status = 'visible' order by created_at desc limit 50", [id]);
    ...
    const store = memoryStore(); return json(res, 200, { items: store.comments.get(id) || [] });
  }
  const body = await readJson(req);
  const wallet = normalizeAddress(body.walletAddress || body.address);
  ...
  const store = memoryStore();
  if (!store.drafts.has(id)) return json(res, 404, { error: "Draft not found" });
  const item = { id: randomUUID(), draftId: id, walletAddress: wallet, body: commentBody, reactionCount: 0, createdAt: new Date().toISOString() };
  const comments = store.comments.get(id) || []; comments.unshift(item); store.comments.set(id, comments);
  const metrics = store.metrics.get(id) || { ...ZERO }; metrics.comments += 1; metrics.signedActions += 1; store.metrics.set(id, metrics);
