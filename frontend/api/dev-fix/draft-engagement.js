...
export async function signedDraftComments(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (pool) {
      ...
      const viewerWallet = normalizeWalletFlexible(query.wallet || query.walletAddress || "") || null;
...
export async function signedDraftCommentReaction(req, res) {
  ...
  // Social reaction: accept EVM or Solana wallet even if draft is on the other chain.
  const wallet = normalizeWalletFlexible(
  ...
  try {
    ...
    // Concurrent double-insert: unique constraint — re-read truth.
