...
async function listPromotors(res) {
...
async function createPromotor(req, res, admin) {
...
async function updatePromotor(req, res) {
...
async function deletePromotor(req, res) {
  const id = String(req.params?.id || "");
  if (!UUID_RE.test(id)) return res.status(400).json({ ok: false, error: "Invalid promoter id." });
...
export async function dashboardPromotors(req, res) {
  ...
  if (req.method === "GET") return await listPromotors(res);
  if (req.method === "POST") return await createPromotor(req, res, admin);
  if (req.method === "PATCH") return await updatePromotor(req, res);
  if (req.method === "DELETE") return await deletePromotor(req, res);
...
export async function dashboardPromotorRefresh(req, res) {
  ...
  if (selected.rowCount === 0) return res.status(404).json({ ok: false, error: "Promoter not found." });
  ...
  try {
    ...
    return res.status(200).json({ ok: true, promotor: updated.rows[0], errors: fetched.errors });
  } catch (error) {
...
export async function dashboardPromotorsRefreshAll(req, res) {
  ...
  for (const row of rows.rows) {
    const fakeReq = { ...req, params: { id: row.id }, method: "POST" };
    let statusCode = 200;
    let payload = null;
    const fakeRes = {
    ...
    await dashboardPromotorRefresh(fakeReq, fakeRes);
    results.push({ id: row.id, status: statusCode, ok: Boolean(payload?.ok), error: payload?.error || null });
  }
