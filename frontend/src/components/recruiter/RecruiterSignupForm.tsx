...
import {
  SOLANA_RECRUITER_CHAIN_ID,
  signRecruiterSolanaMessage,
} from "@/lib/recruiterSolanaSignup";
import {
...
export function RecruiterSignupForm() {
  ...
  const walletMode: "evm" | "solana" | null = isSolanaConnected && solanaAccount ? "solana" : wallet.account ? "evm" : null;
  const account = walletMode === "solana" ? solanaAccount || "" : wallet.account || "";
  ...
  const canSubmit = useMemo(() => {
    const hasSigner = walletMode === "solana" ? true : Boolean(wallet.signer);
    return Boolean(
      isConnected &&
        account &&
        hasSigner &&
  ...
  }, [walletMode, isConnected, account, wallet.signer, form, codeAvailability]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (walletMode === "evm" && !wallet.signer) {
    ...
    try {
      ...
      const signature = walletMode === "solana"
        ? await signRecruiterSolanaMessage(message)
    ...
    return (
      <div className={pageShellClass}>
        <Card className="border-border/60 bg-card/65 p-6">
          ...
          <p className="mt-3 text-sm text-muted-foreground">Use the wallet that should own the recruiter profile.</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <ConnectWalletButton />
            <Button asChild variant="outline" className="font-retro"><Link to="/recruiter">Back to recruiter overview</Link></Button>
          </div>
        </Card>
      </div>
    );
  ...
  if (loadingStatus) {
    return <div className={pageShellClass}><Card className="border-border/60 bg-card/65 px-6 py-12 text-center text-sm text-muted-foreground">Checking recruiter signup status...</Card></div>;
  }
    ...
    return (
      <div className={pageShellClass}>
        <Card className="border-border/60 bg-card/65 p-6">
          ...
          <p className="mt-3 text-sm text-muted-foreground">This wallet already owns recruiter code <span className="text-foreground">{signupStatus.recruiter.code}</span>.</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Button asChild className="font-retro"><Link to="/command/recruiter">Open recruiter dashboard</Link></Button>
            <Button asChild variant="outline" className="font-retro"><Link to={`/recruiters/${encodeURIComponent(signupStatus.recruiter.code)}`}>Public recruiter profile</Link></Button>
          </div>
        </Card>
      </div>
    );
  ...
  return (
    <div className={pageShellClass}>
      <Card className="overflow-hidden border-border/60 bg-[radial-gradient(circle_at_top_left,rgba(240,106,26,0.16),transparent_36%),linear-gradient(180deg,rgba(18,22,28,0.94),rgba(9,12,16,0.98))] p-6 md:p-8">
        ...
        <h1 className="mt-4 font-retro text-3xl text-foreground md:text-5xl">Claim your recruiter identity.</h1>
      </Card>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="border-border/60 bg-card/65 p-6">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2"><Label htmlFor="accept-terms">I confirm this wallet is the recruiter owner and I accept the recruiter program terms.</Label><p className="text-sm text-muted-foreground">Submitting asks your wallet to sign the recruiter signup message.</p></div>
          </div>
          <div className="mt-6 flex flex-wrap gap-3">
