      ...
      <div className="relative flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {wallet.source === "eip6963" ? <span className="rounded-full border border-accent/30 bg-accent/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.18em] text-accent">detected</span> : null}
          </div>
...
export function ConnectWalletModal({ open, onOpenChange, filter }: ConnectWalletModalProps) {
  ...
  const { solanaAccount, availableSolanaWallets, connectingSolana, connectSolana, disconnectSolana } = useSolanaWallet();
  ...
  const statusCopy = useMemo(() => {
    ...
    if (isConnected && account) return `BNB/EVM: ${shortAddress(account)}`;
    const count = evmWallets.length + availableSolanaWallets.length;
    return count > 0 ? `${count} wallet${count === 1 ? "" : "s"} detected` : "No wallet detected yet";
  }, [account, availableSolanaWallets.length, evmWallets.length, isConnected, solanaAccount]);
