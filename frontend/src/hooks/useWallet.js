...
const EIP6963_WALLETS = new Map();
const EIP6963_SUBSCRIBERS = new Set();
...
function getFlag(provider, key) {
    return Boolean(provider[key]);
}
function getProviderMeta(provider, info) {
...
function classifyWallet(provider, info) {
    ...
    if (getFlag(provider, "isOkxWallet") ||
    ...
    if (getFlag(provider, "isMetaMask") ||
...
function dedupeProviders(candidates) {
    const seen = new Set();
    const out = [];
    for (const candidate of candidates) {
        if (!candidate || seen.has(candidate) || typeof candidate.request !== "function")
            continue;
        seen.add(candidate);
        out.push(candidate);
    }
...
function startEip6963Discovery() {
    if (typeof window === "undefined" || eip6963ListenerStarted)
        return;
...
function makeDetectedWallet(provider, source, info) {
    const meta = getProviderMeta(provider, info);
...
function getDetectedWalletsSnapshot() {
    if (typeof window === "undefined")
        return [];
    ...
    for (const wallet of [...eip6963, ...legacy]) {
        ...
        if (providerKey && seenKeys.has(providerKey) && wallet.source === "legacy")
            continue;
        const existingIdCount = seenIds.get(wallet.id) ?? 0;
        const id = existingIdCount > 0 ? `${wallet.id}-${existingIdCount + 1}` : wallet.id;
...
function subscribeToWalletDiscovery(callback) {
    if (typeof window === "undefined")
        return () => undefined;
    startEip6963Discovery();
...
function findDetectedWallet(walletId) {
...
async function choosePrimaryAccount(selectedProvider, accounts) {
    ...
    if (selectedAddress && normalized.includes(selectedAddress))
    ...
    try {
        ...
        if (selectedAddress && active.includes(selectedAddress))
            return selectedAddress;
        if (active[0])
            return active[0];
    }
...
export function useWallet() {
    ...
    const selectedWalletTypeRef = useRef(null);
    const cleanupRef = useRef(null);
    const syncRecruiterAttribution = useCallback(async (walletAddress) => {
        if (!walletAddress)
            return;
        try {
    ...
    const bindEip1193Listeners = useCallback((selectedProvider) => {
        cleanupRef.current?.();
        ...
        const onChainChanged = async (nextChainId) => {
    ...
    const hydrateSelectedProvider = useCallback(async () => {
        ...
        if (explicitlyDisconnected)
            return;
        const storedType = window.localStorage.getItem(SELECTED_WALLET_KEY);
