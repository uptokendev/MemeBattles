...
import { Bell, Edit3, ExternalLink, Flame, Globe, ImageDown, Rocket, Send, Share2, Shield, Star, Users } from "lucide-react";
...
export default function Prepare() {
  ...
  const openShareCard = () => {
    if (!bundle) return;
    window.open(buildShareCardUrl(bundle), "_blank", "noopener,noreferrer");
  };
  ...
  const isCreator = wallet.account && draft.creatorWallet && wallet.account.toLowerCase() === draft.creatorWallet.toLowerCase();
  const links = [
      ...
      <main className="relative z-10">
        <section className="relative flex min-h-[680px] flex-col items-center px-4 py-16 text-center md:px-8 md:py-20">
          ...
          {isCreator && (
            <div className="absolute left-4 top-16 z-20 md:left-auto md:right-4">
              <Button asChild variant="outline" className="mwz-button h-9 px-3 font-retro text-xs">
                <Link to={`/drafts/${draft.id}/promotion`}><Edit3 className="mr-2 h-4 w-4" /> Back to edit</Link>
              </Button>
            </div>
          )}

          <div className="mwz-chip mwz-chip-active mt-8 inline-flex items-center gap-2 px-4 py-2 text-xs md:mt-14">
            <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-orange-300" /> Incoming transmission · Prepare Mode
          </div>

          {draft.logoUrl && <div className="mt-7"><TokenLogo src={draft.logoUrl} ticker={draft.ticker} /></div>}
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            ...
            <Button onClick={share} variant="outline" className="mwz-button h-13 px-6 font-retro text-base"><Share2 className="mr-2 h-4 w-4" /> Share link</Button>
            <Button onClick={openShareCard} variant="outline" className="mwz-button h-13 px-6 font-retro text-base"><ImageDown className="mr-2 h-4 w-4" /> OG PNG</Button>
          </div>
          ...
          <div className="mwz-card mt-12 grid overflow-hidden border-orange-400/35 bg-black/45 md:grid-cols-4">
            {[["Armed recruits", String(pop.signedActions || 0), Users], ["Watchlists", String(followCount ?? pop.follows), Star], ["Heat", `${pop.popularityPercentage}%`, Flame], ["Status", statusLabel(draft.status), Shield]].map(([label, value, Icon], index) => (
              <div key={String(label)} className={`flex items-center gap-3 px-5 py-4 text-left ${index > 0 ? "border-t border-border/50 md:border-l md:border-t-0" : ""}`}>
                ...
                <div><div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">{label as string}</div><div className="mt-1 font-retro text-2xl leading-none text-foreground">{value as string}</div></div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-4 lg:grid-cols-[1.6fr_1fr_1fr]">
            <div className="mwz-card p-6 md:p-8">
              ...
              <p className="mt-4 whitespace-pre-line text-sm leading-7 text-muted-foreground md:text-base">{promo.missionStatement || draft.description || "Creator has not published a mission statement yet."}</p>
            ...
            <div className="mwz-card p-5 md:p-6">
              ...
              <div className="mt-5 flex flex-col gap-2">
                {links.length === 0 ? <p className="text-sm text-muted-foreground">No public comms channels published yet.</p> : links.map(([label, url, meta, code]) => (
                  <a key={String(label)} href={String(url)} target="_blank" rel="noreferrer" className="mwz-button flex items-center justify-between gap-3 px-3 py-3 text-left text-xs">
                    <span className="flex items-center gap-3"><Globe className="h-4 w-4 text-orange-300" /><span><span className="block text-sm text-foreground">{label as string}</span><span className="block text-[10px] text-muted-foreground">{meta as string} · {code as string}</span></span></span><ExternalLink className="h-4 w-4" />
                  </a>
                ))}
              </div>
            </div>
        ...
        <section className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          <div className="mb-6 flex items-center gap-4"><div className="h-px w-16 bg-orange-400/70" /><h2 className="font-retro text-3xl uppercase tracking-[0.12em] text-foreground md:text-4xl">Mission Phases</h2></div>
          <div className="grid gap-3 md:grid-cols-4">
            {fixedMissionPhases().map(([title, body], index) => (
              <div key={title} className={`mwz-card p-5 ${index === 0 ? "border-orange-400/70 bg-orange-500/5" : ""}`}>
                <div className="flex items-center justify-between"><span className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Phase 0{index + 1}</span>{index === 0 ? <Flame className="h-4 w-4 text-orange-300" /> : <Rocket className="h-4 w-4 text-muted-foreground" />}</div>
                <div className="mt-4 font-retro text-3xl uppercase text-foreground">{title}</div>
        ...
        <section className="mx-auto max-w-7xl px-4 py-10 pb-20 md:px-8 md:py-14 md:pb-24">
          <div className="mwz-card border-orange-400/50 bg-[radial-gradient(ellipse_at_top,rgba(255,153,0,0.18),rgba(2,17,4,0.92)_70%)] p-8 text-center md:p-12">
            ...
            <p className="mx-auto mt-4 max-w-xl text-base leading-7 text-muted-foreground">{(followCount ?? pop.follows).toLocaleString()} soldiers already watching. The moment {ticker} moves from draft to live campaign, the alert fires.</p>
            <div className="mx-auto mt-7 flex max-w-xl gap-2"><Input className="h-12 border-border/70 bg-background/50 font-retro" placeholder="wallet or call sign" /><Button onClick={handleFollow} className="mwz-button mwz-button-orange h-12 px-6 font-retro">Arm me</Button></div>
          </div>
        </section>
      </main>
    </div>
