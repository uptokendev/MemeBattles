...
export async function resolveVerifiedTopazRoute(input: {
  ...
  if (market.marketStage !== "TOPAZ_ACTIVE") {
    throw new Error(market.lastError || "Topaz market is not active yet.");
  }
  if (!market.verified || !market.tradingEnabled) {
    throw new Error(market.lastError || "Topaz route is not verified for trading.");
  }
  if (market.stable !== false) throw new Error("MemeWarzone graduation requires a volatile Topaz pool.");

  const campaignAddress = normalizeAddress(input.campaignAddress, "campaign address");
  const tokenAddress = normalizeAddress(market.token, "market token");
  const pairAddress = normalizeAddress(market.pair, "Topaz pair");
  const routerAddress = normalizeAddress(market.router, "Topaz router");
  const factoryAddress = normalizeAddress(market.factory, "Topaz factory");
  const wrappedNativeAddress = normalizeAddress(market.wrappedNative, "wrapped native token");

  if (!sameAddress(market.campaignAddress, campaignAddress)) throw new Error("Market route campaign mismatch.");
